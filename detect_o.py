...
def run(
    ...
    data=ROOT / "data/test.yaml",  # dataset.yaml path
    ...
    model = load_model(weights, device, data, dnn=dnn)
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    if save_txt:  # Write to file
                        ...
                        xyxy2 = [int(x.item()) for x in xyxy]
                        # line = (cls, *xywh, conf) if save_conf else (cls, *xywh)  # label format
                        line = (
                            (cls, *xyxy2, conf) if save_conf else (cls, *xyxy2)
                        )  # label format
...
def parse_opt():
    ...
    parser.add_argument(
        "--conf-thres", type=float, default=0.4, help="confidence threshold"
    )
    parser.add_argument(
